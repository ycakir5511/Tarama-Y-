...
if tarama_modu == "Normal Tarama":
    ...
    st.sidebar.markdown("**Filtre: RSI < 35** (Sabit)")
    rsi_esik = 35
else:
    st.sidebar.info("Altın Vuruş: 1S, 4S ve Günlük periyotların hepsinde RSI < 40 olanları bulur.")
    rsi_esik = 40
...
if baslat:
    ...
    if sonuclar:
        ...
        st.success(f"Analiz tamamlandı! {len(df)} hisse bulundu.")
        st.dataframe(df, use_container_width=True)
        
        csv = df.to_csv(index=False).encode('utf-8')
        st.download_button("📥 Listeyi İndir (CSV)", csv, "bist_tarama.csv", "text/csv")
    else:
        st.warning(f"Kriterlere (RSI < {rsi_esik}) uygun hisse bulunamadı.")
else:
    st.info("Ayarlarınızı seçtikten sonra 'Analizi Başlat' butonuna basın.")
